...
def _make_telegram_bot():
    ...
    # Manually set required instance vars (mirroring _make_telegram_bot in test_put_approval.py)
    ...
    # Call approval state (new — these will fail until implemented)
...
class TestCallApprovalFlow:
    ...
    def test_approve_places_call_order(self):
        ...
        async def run():
            async def fake_wait_for(coro, timeout):
                bot._call_approval_result = "approved"

            with patch.object(bot, "_execute_call_order", new=AsyncMock(return_value=True)) as mock_exec:
    ...
    def test_reject_cancels_without_order(self):
        ...
        async def run():
            async def fake_wait_for(coro, timeout):
                bot._call_approval_result = "rejected"

            with patch.object(bot, "_execute_call_order", new=AsyncMock(return_value=False)) as mock_exec:
                with patch("asyncio.wait_for", new=fake_wait_for):
                    result = await bot.request_call_approval(signal, chain, mock_client, mock_db, "acc")
    ...
    def test_separate_call_approval_event(self):
        """_call_approval_event must be a different object from _put_approval_event and _approval_event."""
        bot = _make_telegram_bot()
        signal = _make_call_signal()
        chain = _make_call_chain()
        ...
        bot._put_approval_event = put_event
        ...
        async def run():
            ...
            # _call_approval_event was created (a new Event, different from intraday and put)
            assert bot._call_approval_event is not None
            assert bot._call_approval_event is not intraday_event
            assert bot._call_approval_event is not put_event
...
class TestCallAdjust:
    ...
    def test_shows_alternatives_above_cost_basis(self):
        ...
        bot._call_approval_signal = signal
        bot._call_approval_chain = chain
    ...
    def test_no_alternatives_sends_warning(self):
        ...
        assert "cost basis" in text.lower() or "no profitable" in text.lower() or "above" in text.lower()
...
class TestCallbackRouting:
    ...
    def test_call_approve_prefix_dispatched(self):
        """call_approve_ callback sets _call_approval_result='approved' and fires event."""
        bot = _make_telegram_bot()
        event = asyncio.Event()
        ...
        assert bot._call_approval_result == "approved"
        assert event.is_set()

    def test_call_reject_prefix_dispatched(self):
        """call_reject_ callback sets _call_approval_result='rejected' and fires event."""
        bot = _make_telegram_bot()
        event = asyncio.Event()
        bot._call_approval_event = event
        bot._call_approval_callback_id = "103000"
        ...
        assert bot._call_approval_result == "rejected"
        assert event.is_set()

    def test_call_prefix_before_intraday(self):
        """call_reject_ does NOT trigger _approval_result (intraday); only _call_approval_result."""
        bot = _make_telegram_bot()
        event = asyncio.Event()
        bot._call_approval_event = event
        bot._call_approval_callback_id = "103000"
        ...
        # call_* must set _call_approval_result
        assert bot._call_approval_result == "rejected"
    ...
    def test_call_alt_sets_strike_result(self):
        """call_alt_ callback extracts strike and sets it as _call_approval_result."""
        bot = _make_telegram_bot()
        event = asyncio.Event()
        bot._call_approval_event = event
        bot._call_approval_callback_id = "103000"
        ...
        assert bot._call_approval_result == "52.0"
        assert event.is_set()

    def test_call_alt_reject_sets_rejected(self):
        """call_alt_reject_ callback sets _call_approval_result='rejected' and fires event."""
        bot = _make_telegram_bot()
        event = asyncio.Event()
        bot._call_approval_event = event
        bot._call_approval_callback_id = "103000"
...
class TestSchedulerCallSuggestion:
    ...
    def test_suggests_call_after_otm_expiry(self):
        ...
        call_signal = _make_call_signal()
        scheduler = self._make_scheduler(expiry_result="call_expired_otm", call_signal_result=call_signal)
    ...
    def test_no_profitable_strikes_sends_warning(self):
        ...
        assert "no profitable" in notification_msg.lower() or "cost basis" in notification_msg.lower()
    ...
    def test_called_away_sends_summary(self):
        ...
        assert "P&L" in notification_msg or "pnl" in notification_msg.lower() or "$330" in notification_msg
