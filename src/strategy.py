...
from .utils import (
    get_et_now, get_market_times, is_market_day, is_monday, is_friday,
    is_in_dip_window, calculate_dip_percentage, calculate_shares,
    calculate_pnl, is_trading_day, get_day_of_week
)
from .database import Database, get_database
from .etrade_client import ETradeClient, ETradeAPIError
...
class StrategyConfig:
    ...
    # Strategy selection
    strategy_type: str = "combined"  # "original_dip", "mean_reversion", "short_thursday", "combined"
...
class IBITDipStrategy:
    ...
    def analyze(self) -> TradeSignal:
        ...
        if state.is_paused:
            return TradeSignal(
                ...
                reason=f"Bot is paused: {state.pause_reason}"
            )
        ...
        if state.has_position:
            ...
            if state.current_price and state.position_entry_price:
                unrealized_pnl = ((state.current_price - state.position_entry_price)
                                  / state.position_entry_price * 100)
            return TradeSignal(
                action=TradeAction.HOLD,
        ...
        if not is_in_dip_window():
            if now < times["dip_window_start"]:
                return TradeSignal(
                    ...
                    reason=f"Waiting for dip window (10:00 AM ET)",
                    dip_percentage=state.dip_percentage
                )
            else:
                return TradeSignal(
    ...
    def _execute_buy(self, signal: TradeSignal) -> Dict[str, Any]:
        ...
        preview = self.client.preview_order(
            ...
            limit_price=signal.price * (1 + self.config.limit_offset_pct / 100) if self.config.use_limit_orders else None
        )
        ...
        # Log event
        self.db.log_event("INFO", "BUY executed", {
            "trade_id": trade_id,
            "order_id": order_id,
            "shares": signal.shares,
            "price": signal.price,
    ...
    def _execute_sell(self, signal: TradeSignal) -> Dict[str, Any]:
        ...
        # Log event
        self.db.log_event("INFO", "SELL executed", {
            "trade_id": open_trade["id"],
            "order_id": order_id,
            "shares": signal.shares,
